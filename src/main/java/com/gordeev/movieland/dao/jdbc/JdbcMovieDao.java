...
public class JdbcMovieDao implements MovieDao {
    ...
    public List<Movie> getAll(String column, String direction) {
        logger.info("Start processing query to get all movies ordered by column: {} and direction is: {}", column, direction);
        long startTime = System.currentTimeMillis();
        String sql = GET_ALL_ORDERED_MOVIE_SQL + " " + column + " " + direction;

        List<Movie> movies = jdbcTemplate.query(sql, MOVIE_ROW_MAPPER);

        logger.info("Finish processing query to get all movies ordered by column: {} and direction is: {}. It took {} ms", column, direction, System.currentTimeMillis() - startTime);
        return movies;
    }
